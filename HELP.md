...
Always check this before implementing database-related functionality.

docs/api.yaml

Authoritative REST API contract.

Always check this before implementing or modifying an API.
...
Update the DTOs if required.
Update the controller.
Update the service.
Update tests.
Verify the implementation against docs/api.yaml.
Database Changes

When changing the database:

Update docs/database-schema.mmd.
Create a new Flyway migration.
Update JPA entities.
Update repositories if required.
Update affected services/APIs.
